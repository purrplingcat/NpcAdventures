...
namespace NpcAdventure.AI
{
    internal partial class AI_StateMachine
    {
        ...
        {
            if (this.medkits > 0 && this.farmer.health < (this.farmer.maxHealth / 3) && this.farmer.health > 0)
            {
                float healthBonus = (this.farmer.maxHealth / 100) * (this.farmer.getFriendshipHeartLevelForNPC(this.npc.Name) / 2); // % health bonus based on friendship hearts
                float medkitPower = 1 + (this.farmer.FarmingLevel * (this.farmer.ForagingLevel + 1)) / 100; 
                float baseHealth = Math.Max(10, (1 / this.farmer.health * 10) + Game1.random.Next(0, (int)(this.farmer.maxHealth * .1f))) + healthBonus;
                int health = (int)(baseHealth * medkitPower);
                this.farmer.health += health;
                this.healCooldown = HEAL_COUNTDOWN;
                this.medkits--;

                if (this.farmer.health > this.farmer.maxHealth)
                    this.farmer.health = this.farmer.maxHealth;

                Game1.drawDialogue(this.npc, this.Csm.Dialogues.GetFriendSpecificDialogueText(this.farmer, "heal"));
                ...
                this.farmer.currentLocation.playSound("healSound");
            ...
            if (this.medkits == 0)
            {
                ...
                Game1.drawDialogue(this.npc, this.Csm.Dialogues.GetFriendSpecificDialogueText(this.farmer, "nomedkits"));
                this.medkits = -1;
            }
        ...
        {
            float npcPlayerDistance = Helper.Distance(this.farmer.GetBoundingBox().Center, this.npc.GetBoundingBox().Center);
            bool noMonstersNearPlayer = Helper.GetNearestMonsterToCharacter(this.farmer, 4f) == null;

            if (this.farmer.health <= 0 || npcPlayerDistance > 2.25 * Game1.tileSize || noMonstersNearPlayer)
                return;

            double chance = Math.Max(0.01, (this.farmer.DailyLuck / 2.0 + this.farmer.LuckLevel / 100.0 + this.farmer.getFriendshipHeartLevelForNPC(this.npc.Name) * 0.05));
        ...
        public void UpdateDoctor(UpdateTickedEventArgs e)
        {
            ...
            if (this.healCooldown > 0 && Context.IsPlayerFree)
            {
                ...
                if (e.IsMultipleOf(180) && (this.healCooldown > HEAL_COUNTDOWN * .7f) && this.farmer.health < (this.farmer.maxHealth * .61f))
                    this.farmer.health += Math.Max(1, (int)Math.Round(this.farmer.maxHealth * .01f)) + (int)Math.Round(this.farmer.getFriendshipHeartLevelForNPC(this.npc.Name) / 4f);

                this.healCooldown--;
            }

            // Doctor companion try to save your life if you have less than 5% of health and your life not saved in last time
            if (e.IsOneSecond && this.medkits > 0 && this.farmer.health < this.farmer.maxHealth * 0.05 && !this.lifeSaved)
                this.TrySaveLife();
        }
    }
}
